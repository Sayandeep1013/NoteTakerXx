...
export default function NoteContextMenu({ x, y, noteId, noteBadges, onEdit, onDelete, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    return () => { clearTimeout(tid); window.removeEventListener("keydown", onKey); window.removeEventListener("pointerdown", onClick); };
  }, [onClose]);
  ...
  const cx = Math.min(x, vw - 196);
  const cy = y + estimatedH > vh - 8 ? y - estimatedH : y;

  // Match the sidebar glassmorphism aesthetic
  ...
  return (
    <div
      ref={menuRef}
      style={{
        position: "fixed", left: cx, top: cy, zIndex: 800,
        background: bg,
        ...
        borderRadius: 12,
        boxShadow: isDark
          ? "0 2px 8px rgba(0,0,0,0.5), 0 16px 40px rgba(0,0,0,0.55)"
          : "0 2px 6px rgba(0,0,0,0.10), 0 12px 32px rgba(0,0,0,0.12)",
    ...
    >
      ...
      {/* Add badge — hover reveals side submenu */}
      <div
      ...
      >
        <Item
          ...
          icon={<IcoBadge />}
          txt={txt} hov={hov}
          suffix={<svg width="8" height="8" viewBox="0 0 8 8" fill="none"><path d="M2 1.5l3.5 3-3.5 3" stroke={muted} strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round"/></svg>}
          onClick={() => {}}
        />
        {badgesHovered && (
          <div style={{
            ...
            padding: 8, display: "flex", flexWrap: "wrap", gap: 5, width: 130, zIndex: 10,
            animation: "ctxPop 80ms cubic-bezier(0.2,0,0,1.2)",
          }}>
            {allBadges.map(({ id, label, Icon }) => {
              const active = noteBadges.includes(id);
              return (
                <button key={id} title={active ? `Remove "${label}"` : label}
                  onClick={() => { toggleNoteBadge(noteId, id); onClose(); }}
                  style={{ width: 36, height: 36, borderRadius: 9, padding: 4, border: "none", background: active ? (isDark ? "rgba(255,255,255,0.12)" : "rgba(0,0,0,0.08)") : "transparent", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", outline: active ? `1.5px solid ${isDark ? "rgba(255,255,255,0.18)" : "rgba(0,0,0,0.14)"}` : "1.5px solid transparent", transition: "all 80ms" }}
            ...
            {noteBadges.length > 0 && <div style={{ width: "100%", fontSize: 10, color: muted, textAlign: "center", marginTop: 2 }}>hover to remove</div>}
          </div>
        )}
      </div>
